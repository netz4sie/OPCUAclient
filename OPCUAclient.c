...
handler_TheAnswerChanged(UA_Client *client, UA_UInt32 subId, void *subContext,
    ...
    printf("The Answer has changed! A gdzie to?\n");
}
#endif
...
int main(int argc, char *argv[]) {
    ...
    UA_StatusCode retval = UA_Client_getEndpoints(client, "opc.tcp://localhost:48010",
    ...
    //retval = UA_Client_connectUsername(client, "opc.tcp://localhost:48010", "user1", "password");
...
//    bReq.nodesToBrowse[0].nodeId = UA_NODEID_NUMERIC(UA_NODEID_STRING(2,"DeviceSet"),UA_NS0ID_OBJECTSFOLDER); /* browse objects folder */
    bReq.nodesToBrowse[0].nodeId = UA_NODEID_NUMERIC(3, 5027); /* browse objects folder */
    bReq.nodesToBrowse[0].resultMask = UA_BROWSERESULTMASK_ALL; /* return everything */
//    bReq.nodesToBrowse[1].nodeId = UA_NODEID_NUMERIC(3, 5025); /* browse objects folder */
//    bReq.nodesToBrowse[1].resultMask = UA_BROWSERESULTMASK_ALL; /* return everything */
    UA_BrowseResponse bResp = UA_Client_Service_browse(client, bReq);
    printf("%-9s %-16s %-42s %-42s\n", "NAMESPACE", "NODEID", "BROWSE NAME", "DISPLAY NAME");
    for(size_t i = 0; i < bResp.resultsSize; ++i) {
        for(size_t j = 0; j < bResp.results[i].referencesSize; ++j) {
            UA_ReferenceDescription *ref = &(bResp.results[i].references[j]);
            if(ref->nodeId.nodeId.identifierType == UA_NODEIDTYPE_NUMERIC) {
                printf("%-9u %-16u %-42.*s %-42.*s\n", ref->nodeId.nodeId.namespaceIndex,
                       ref->nodeId.nodeId.identifier.numeric, (int)ref->browseName.name.length,
                       ref->browseName.name.data, (int)ref->displayName.text.length,
                       ref->displayName.text.data );
            } else if(ref->nodeId.nodeId.identifierType == UA_NODEIDTYPE_STRING) {
                printf("%-9u %-16.*s %-42.*s %-32.*s\n", ref->nodeId.nodeId.namespaceIndex,
    ...
    *parent = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
    ...
    UA_Client_forEachChildNodeCall(client, UA_NODEID_NUMERIC(3, 5025),
    ...
    UA_MonitoredItemCreateRequest monRequest =
        UA_MonitoredItemCreateRequest_default(UA_NODEID_NUMERIC(4,104438));
...
//    UA_Int32 value = 0;
    UA_Float value = 0;
    printf("\nReading the value of node 4, Mains power (real part):\n");
    UA_Variant *val = UA_Variant_new();
    retval = UA_Client_readValueAttribute(client, UA_NODEID_NUMERIC(4,104607), val);
    //printf("the value is: %f\n", *(UA_Double*)val->data);
    value = *(UA_Float*)val->data;
    printf("tttthe value is: %f\n", value);
